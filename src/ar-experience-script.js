...
*/
    ...
    if (arObjects.length > 0) {
        ...
        if (pathwaysReady) {
            ...
            if (activePath < curveCount) {
                const nextPos = pathways[activePath].getPoint(animTimer);
                const movDir = new THREE.Vector3(
                    nextPos.x - model.position.x,
                    nextPos.y - model.position.y,
                    nextPos.z - model.position.z
                );
                // const movDir = 
                movDir.normalize();
    ...
    if (animationMixer_2) {
        ...
        if (elapsedTime > animationInformation[currentAnimation + 1][0]) {
            if (currentAnimation > -1) {
                pathwayAnimations[animationInformation[currentAnimation][1]].stop();
            }
            currentAnimation += 1;
            ...
            if (currentAnimation > 0) {
                ...
                pathwayAnimations[animationInformation[currentAnimation-1][1]].fadeOut(1);
                pathwayAnimations[animationInformation[currentAnimation][1]].fadeIn(1);
            }
            pathwayAnimations[animationInformation[currentAnimation][1]].play();

            document.getElementById("active-animation").textContent = animationInformation[currentAnimation][1];
        }
    }
...
function generatePaths() {
    for (const [curveName, curveInfo] of Object.entries(sceneInformation)) {
        ...
        pathwayDurations.push(1 / curveInfo["duration"]);
    }
